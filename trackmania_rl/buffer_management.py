...
class Pinned_buffer_async:
    def __init__(
        self,
        pinned_buffer_size: int,
        pinned_buffer_Lock
    ) -> None:
        self.pinned_buffer = torch.empty((pinned_buffer_size, 1, misc.H_downsized, misc.W_downsized), dtype=torch.uint8)
        torch.cuda.cudart().cudaHostRegister(
        self.pinned_buffer.data_ptr(), pinned_buffer_size * misc.H_downsized * misc.W_downsized, 0
        )
        ...
        for reciever in self.Recievers:
            for i in range(0,reciever.qsize()):
                try:
                    ...
                    self.pinned_buffer[data[0]]=data[1]
                except:
                    pass

    def get_frames_from_buffer(self,idx):
        self.CheckRecievers()
        return self.pinned_buffer[idx]
...
def fill_buffer_from_rollout_with_n_steps_rule_async(
    buffer_pipe: mp.Queue,
    buffer_test_pipe: mp.Queue,
    rollout_results: dict,
    n_steps_max: int,
    gamma: float,
    discard_non_greedy_actions_in_nsteps: bool,
    ...
    number_memories_added = 0
    buffer_pipe_to_fill = buffer_test_pipe if random.random() < misc.buffer_test_ratio else buffer_pipe
