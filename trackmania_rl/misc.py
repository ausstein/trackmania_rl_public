...
use_eval_runs_in_buffer=True  ##made this optional
use_random_exponential_epsilon =True  ##draw the epsilons from random exponential variables with epsilon mean instead of using the same epsilons every time
tm_engine_step_per_action_eval =5 ##not implemented but want to in the future
if not tm_engine_step_per_action_eval==tm_engine_step_per_action:
    use_eval_runs_in_buffer=False

#if np.random.rand()<0.3: ##was my solution to varying epsilon before doing the exponential random variable solution
#    epsilon = 0.3
#    epsilon_boltzmann = 0.3
#else:
epsilon = 0.01
...
buffer_test_ratio = 0.02
write_worker_prints_to_file = True #if true the main console only shows prints from main process
...
number_memories_generated_high_exploration_early_training = 200_000
...
number_memories_trained_on_between_target_network_updates = 10000*1

soft_update_tau = 0.01*1
